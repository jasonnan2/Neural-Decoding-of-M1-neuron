...
function [Parameters] = setThetaAug(train, T)
% FIX NORMALIZATION
N=91;K=8;
...
for r = 1:5
    for i = 1:length(Num_trials)
        ...
        for j = 1:Num_trials(i)
            for a = 1:20
                for t= 2:T{r,i}(j)
                    zt = train(r,i).data(j,a).arm(:,t);
                    zt1 =  train(r,i).data(j,a).arm(:,t-1);
                    top = top+zt*zt1';
...
for r=1:5
    for i=1:length(Num_trials)
        ...
        for j = 1:Num_trials(i)
            for a = 1:20
                for t = 1:T{r,i}(j)
                    xt= train(r,i).data(j,a).spikes(:,t);
                    zt= train(r,i).data(j,a).arm(:,t);
                    top=top+xt*zt';
                    bottom=bottom+zt*zt';
                end            
        ...
        C = top*inv(bottom);
        C(C==0) = 0.001;
        Parameters(r,i).C=Parameters(r,i).C+C;
    end
end
...
for r=1:5
    for i=1:length(Num_trials)
        ...
        for j = 1:Num_trials(i)
            for a = 1:20
                Parameters(r,i).init=[Parameters(r,i).init,train(r,i).data(j,a).arm(:,1)];
            end
        end
